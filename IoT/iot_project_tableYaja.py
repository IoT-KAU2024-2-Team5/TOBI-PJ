...
def update_led_and_pump_in_state_table(plant_id, des_led, des_pump):
    ...
    print(f"State table updated successfully for id: {"테이블야자"}, LED: {des_led}, Pump: {des_pump}")
    return response
...
def lambda_handler(event, context):
    try:
        ...
        # 상태 테이블에서 이전 값 가져오기
        db_response = state_table.get_item(Key={"id": "테이블야자"})
        ...
        print(f"State table {state_table_name} updated successfully for id: 테이블야자")
        ...
        try:
            ...
            if mode == 'auto':
                ...
                desired_led = hdl_led('테이블야자', brightness)
                desired_pump = hdl_pump('테이블야자', humidity)

                update_led_and_pump_in_state_table("테이블야자", desired_led, desired_pump)
