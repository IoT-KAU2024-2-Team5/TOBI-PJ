...
def update_led_and_pump_in_state_table(plant_id, des_led, des_pump):
    ...
    print(f"State table updated successfully for id: {"스킨답서스"}, LED: {des_led}, Pump: {des_pump}")
    return response
...
def lambda_handler(event, context):
    try:
        ...
        # 상태 테이블에서 이전 값 가져오기
        db_response = state_table.get_item(Key={"id": "스킨답서스"})
        ...
        print(f"State table {state_table_name} updated successfully for id: 스킨답서스")
        ...
        try:
            ...
            if mode == 'auto':
                ...
                desired_led = hdl_led('스킨답서스', brightness)
                desired_pump = hdl_pump('스킨답서스', humidity)

                update_led_and_pump_in_state_table("스킨답서스", desired_led, desired_pump)
